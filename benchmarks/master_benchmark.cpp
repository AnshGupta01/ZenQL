...
static void batch_insert(FlexQL* db, const std::string& table,
    ...
    for (int i = 0; i < (int)rows.size(); i++) {
        ...
        bool flush = (i + 1) % batch_size == 0 || i == (int)rows.size() - 1;
        if (flush) {
            flexql_exec(db, buf.c_str(), nullptr, nullptr, &err);
            buf.clear();
        }
        if (progress_every > 0 && i > 0 && i % progress_every == 0)
            std::cout << "  [" << table << "] " << i << " rows...\n";
    }
}
...
static Result phase_insert(FlexQL* db, int N) {
    ...
    auto t0 = Clock::now();
    batch_insert(db, "P1", "", vals, 500, N / 10);
...
static Result phase_mixed(FlexQL* db, int N) {
    ...
    std::thread reader([&]() {
        FlexQL* rdb = nullptr;
        flexql_open("127.0.0.1", 9000, &rdb);
        int probe = 0;
    ...
    for (int i = 0; i < N; i++) {
        ...
        if ((i + 1) % 400 == 0 || i == N - 1) {
            flexql_exec(db, row_batch.c_str(), nullptr, nullptr, &err);
            row_batch.clear();
        }
        if (i > 0 && i % (N / 10) == 0)
            std::cout << "  [P2] " << i << " rows (reads so far: "
                      << read_ops.load() << ")\n";
    }
...
static Result phase_query(FlexQL* db, int N, int Q) {
    ...
    batch_insert(db, "P3", "", vals, 500, N / 10);
    ...
    for (int i = 0; i < Q; i++) {
        ...
        if ((i + 1) % 200 == 0 || i == Q - 1) {
            flexql_exec(db, batch.c_str(), null_cb, nullptr, &err);
            batch.clear();
        }
        if (i > 0 && i % (Q / 10) == 0)
            std::cout << "  [P3] " << i << " queries done...\n";
    }
    double elapsed = Sec(Clock::now() - t0).count();
...
static Result phase_join(FlexQL* db, int emp_count, int dept_count, int iters) {
    ...
    batch_insert(db, "EMPS2", "", ev, 500, emp_count / 10);
...
int main(int argc, char** argv) {
    ...
    FlexQL* db = nullptr;
    check(flexql_open("127.0.0.1", 9000, &db), "connect to FlexQL server");
    std::cout << "  ✓ Connected\n";
